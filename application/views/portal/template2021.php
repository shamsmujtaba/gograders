...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<?= asset_url() ?>root_css/custom.css" />
    ...
    <link rel="icon" href="<?= asset_url() ?>front/images/faviconacd.png" type="image/gif" sizes="16x16">
    <!-- For loader -->
...
<body>
    ...
    <script>
        ...
        var previousContent = $('.iti__selected-dial-code').text();
        setInterval(function() {
            var currentContent = $('.iti__selected-dial-code').text();
    ...
    <script>
        var serverError= true; // To track server Error
        ...
        function applyDiscount(price) {
            ...
            $('#extra-disscount').html('10% Extra Discount Applied');
        ...
        function saveData(is_phone, element_id) {
            ...
            halfPrice = convertPrice(acedamic_level, date, time, pages, currencyType);
             var total_pbd= calculateActual60off(halfPrice)
            if (phone.match(phoneno)) {
            ...
            if (phone === "" ||
                ...
                $('#rev-price').text('$0');
                $('.actual').text('$0');
                $('#extra-disscount').html('Calculating 10% discount');
                ...
                if (phone.match(phoneno)) {
                    if ($("#paymentStatus").prop("checked") == true) {
                        ...
                        $(".total_pbd").html(total_pbd.toFixed(2));
                        $('#pay-only').show();
       ...
       function calculateActual60off(price){
            var isApplicable = $('#check-step').val();
            if (isApplicable === 'yes') { 
                tp=(price *10)/100
                price= price+tp
    ...
    <script>
        function saveLeads(stepNum, is_phone) {
            ...
            let discount = 0;
            let isApplicable = $('#check-step').val();
            if (isApplicable === 'yes') {
                discount = 1;
            }
        ...
        function generateLead(formdata) {
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    $("#lead-loader").show();
                    if (lead_id == 0) {
                        // $('#extra-disscount').html('Applying 10% Extra Discount');
                        $('.flip-button__front .fa-arrow-right').hide()
                ...
                complete: function() {
                    ...
                    $("#lead-loader").hide();
                    $('.flip-button__front .fa-arrow-right').show()
                    if (lead_id == 0) {
                    $('#extra-disscount').html('Add 10% Extra Discount');
    ...
    //  ------------Flip Button-------------------
    function flipButton() {
    const button = document.getElementById('save-lead-button');
    button.classList.remove('flip-animation'); // Remove the animation class
    void button.offsetWidth; // Trigger reflow to reset the animation
    button.classList.add('flip-animation'); // Add the animation class back
}
